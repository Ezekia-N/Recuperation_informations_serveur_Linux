...
class Information:
    def __init__(self):
        self.output = subprocess.run(['../bash/system_identity.sh'], capture_output=True, text=True)
        self.systemID = json.loads(self.output.stdout)

        self.fs_json = subprocess.run(["../bash/file_system.sh"], text=True, capture_output=True)
        self.fs = json.loads(self.fs_json.stdout)

        self.ram = ctypes.CDLL(os.path.abspath("../../lib/libram_usage.so"))
        self.cpu = ctypes.CDLL(os.path.abspath("../../lib/libcpu_usage.so"))
...
class Widget(QWidget):
    ...
    def ram(self):
        ...
        label.setText(f"<b>{self.ram_pourcentage:.2f}%</b> utilisé")
    ...
    def ressource_usage(self):
        ...
        self.ui.gridLayout_ressource.addWidget(QLabel("graphe", styleSheet=self.font_style), 0, 1)
        self.ui.gridLayout_ressource.addWidget(QLabel(f"{self.info.cpu.get_cpu_usage():.2f}%", styleSheet=self.font_style), 0, 2)
        self.ui.gridLayout_ressource.addWidget(QLabel("RAM", styleSheet=self.font_style), 1, 0)
        self.ui.gridLayout_ressource.addWidget(QLabel("graphe", styleSheet=self.font_style), 1, 1)
        self.ui.gridLayout_ressource.addWidget(QLabel(f"{self.info.ram.get_ram_usage():.2f}%", styleSheet=self.font_style), 1, 2)
